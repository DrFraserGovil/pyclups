...
\begin{document}
	...
	\section{Prediction Errors}
		...
		\begin{spalign}
			H_{ij} & = \pdiv{^2\mathcal{L}}{\hat{X}_i \hat{X}_j}
			\\
			& = \pdiv{}{\hat{X}_j} \left( \pdiv{\mathcal{L}}{\ai} \cdot \vec{X}\right)
			\\
			& =  \pdiv{}{\hat{X}_j} \left( \left[2 K \ai - 2 \ki \cdot \ai\right] \cdot \vec{X}\right)
			\\
			& = 2 \vec{X} \cdot K \vec{X}
		\end{spalign}
		Therefore, the BLP error is constant and equal to:
		\begin{equation}
			\sigma = \frac{1}{\sqrt{2 \vec{X} \cdot K \vec{X}}}
		\end{equation}
\end{document}