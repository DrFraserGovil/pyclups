...
\begin{document}
	...
	\section{The CLUP}
		...
		\subsection{The CLUP}
			...
			\begin{spalign}
				...
				\\
				& = \ai^\text{blup} + \frac{\left(D^T \left(D D^T \right)^{-1} \left(\vec{c} - D A^\text{clup}\vec{X}\right)\right)\cdot \hat{e}_i}{\beta} B K^{-1} \vec{X}\label{E:BLUP_A}
			\end{spalign}
			Where:
			\begin{spalign}
			...
			% We note that in the case where $\text{dim}(\Phi) = 0$ (i.e. the case where we do not allow any approximation to the mean function), then $C = 0$, $B^i = \mathds{1}, \alpha_i \to A_i$, and so:
			% \begin{spalign}
			% 	\ell_i \to 0 
			% \end{spalign}
			% We see that we have recovered the CLP solution, in the case where $g(t) = 0$ -- exactly as we might naively expect.
	...
	\subsection{Deriving (Again)}
		...
		We begin with the formulation of \eref{E:BLUP_A}, with some notational changes:
		\begin{spalign}
			\ai^\text{clup} & = K^{-1} \Delta \vec{k}_i + C \vec{\varphi}_i + \frac{\left(D^T (DD^T)^{-1} (\vec{c} - D \vec{\alpha})\right)\cdot \hat{e}_i}{\beta} K^{-1} \Delta \vec{X}
			\\
			& = \ai^\text{blup} + s_i K^{-1} \Delta \vec{X}
		\end{spalign}
		The matrix $\Delta$ is the `residual' matrix, such that:
		\begin{spalign}
			\Delta & = 1 - \Phi^T (\Phi K^{-1} \Phi^T)^{-1} \Phi K^{-1}
			\\
			s_i & = \frac{1}{\beta}\left(D^T (DD^T)^{-1} (\vec{c} - D \vec{\alpha})\right)\cdot \hat{e}_i
		\end{spalign}
		...
		The properties of the outer product are such that:
		\begin{spalign}
			\left(\vec{u} \otimes \vec{v}\right) \vec{w} & = (\vec{v} \cdot \vec{w}) \vec{u}
			\\
			\left(\vec{u} \otimes \vec{v}\right)^T & = \left(\vec{v} \otimes \vec{u}\right)
		\end{spalign}
		Therefore we can write (using $H = D^T (DD^T)^{-1}$)
		\begin{spalign}
			\left(H (\vec{c} - D \vec{\alpha})\right)\cdot \hat{e}_iK^{-1} \Delta \vec{X} & = \left( (K^{-1} \Delta \vec{X}) \otimes \hat{e}_i \right) \left(H (\vec{c} - D \vec{\alpha})\right)
		\end{spalign}
		So:
		\begin{spalign}
			\ai^\text{clup} & = \ai^\text{blup} + \left( (K^{-1} \Delta \vec{X}) \otimes \hat{e}_i \right) \left(H (\vec{c} - D \vec{\alpha})\right)
		\end{spalign}
		This has the corollary that:
		\begin{spalign}
			\pdiv{\ai^\text{clup}}{\vec{c}} = \left( (K^{-1} \Delta \vec{X}) \otimes \hat{e}_i \right) H
		\end{spalign}
		In order to find the optimal value of $\vec{c}$ we need to optimise the Lagrangrian w.r.t the parameters of $\vec{c}$, so:
		\begin{spalign}
			\pdiv{\mathcal{L}}{z_k} & = \sum_i \pdiv{\mathcal{L}}{\ai} \cdot \pdiv{\ai}{z_k}
			\\
			& = \sum_i \pdiv{\mathcal{L}}{\ai} \cdot \pdiv{\ai}{\vec{c}} \pdiv{\vec{c}}{z_k}
			\\
			& = \sum_i 2\left( K \ai - \vec{k}_i \right) \cdot   \left[ (K^{-1} \Delta \vec{X}) \otimes \hat{e}_i \right]  \left(H \pdiv{\vec{c}}{z_k} \right)
			\\
			& = \sum_i 2 \left[ (K^{-1} \Delta \vec{X}) \otimes \hat{e}_i \right]^T \left( K \ai - \vec{k}_i \right) \cdot    \left(H \pdiv{\vec{c}}{z_k} \right)
		\end{spalign}
		We can now continue to exploit the properties of the inner products to simply this equation:
		\begin{spalign}
			\pdiv{\mathcal{L}}{z_k}& = \sum_i 2 \left[\hat{e}_i  \otimes  (K^{-1} \Delta \vec{X}) \right] \left( K \ai - \vec{k}_i \right) \cdot    \left(H \pdiv{\vec{c}}{z_k} \right)
			\\
			& = 2 \sum_i \left( \left[K^{-1} \Delta \vec{X} \cdot K \ai \right] \hat{e}_i - \left[K^{-1} \Delta \vec{X} \cdot \vec{k}_i \right] \hat{e}_i \right) \cdot \left(H \pdiv{\vec{c}}{z_k} \right)
			\\
			& = 2 \left[\sum_i \left( \vec{\delta} \cdot \ai - \vec{\delta} \cdot  \ai^{blp} \right) \hat{e}_i \right] \cdot H \pdiv{\vec{c}}{z_k}
		\end{spalign}
		Where $\vec{\delta} = \Delta \vec{X}$. 

		\subsubsection{Sanity Check: Recovering the BLUP}

			We should wish to note that in the case where the BLUP obeys the constraints, the CLUP $\to$ BLUP. When this is the case, we have $D \vec{\alpha} = \vec{c}$, where $\alpha_i = \ai^{blup} \cdot \vec{X}$, and so:
			\begin{spalign}
				\ai^\text{clup} & = \ai^\text{blup} + \vec{0}
				\\
				& = K^{-1} \Delta \vec{k}_i + C \vec{\varphi}_i 
			\end{spalign}
			We also recall that, by definition of the BLUP and the CLUP, $\Phi \ai^\text{blup} = \Phi \ai^\text{clup} = \varphi_i$, hence - Rearranging:
			\begin{spalign}
				\delta \cdot \ai^\text{clup} & = \delta \cdot \ai^\text{blup}
				\\
				& = \vec{X} \cdot \Delta^T \ai^\text{blup}
				\\
				& = \vec{X} \cdot (1 - C \Phi) \ai^\text{blup}
				\\
				& = \vec{X} \cdot (\ai^\text{blup} - C \vec{\varphi}_i )
				\\
				& = \vec{X} \cdot (K^{-1} \Delta \vec{k}_i)
				\\
				& = \delta \cdot K^{-1} \vec{k}_i
				\\
				& = \delta \cdot \ai^\text{blp}
			\end{spalign}
			Therefore, when the BLUP obeys the constraints, we find that:
			\begin{spalign}
				\left.\pdiv{\mathcal{L}}{z_k} \right|_{BLUP}& = 2 \left[\sum_i \left( \vec{\delta} \cdot \ai^{BLUP} - \vec{\delta} \cdot  \ai^{blp} \right) \hat{e}_i \right] \cdot H \pdiv{\vec{c}}{z_k}
				\\
				& =  2 \left[\sum_i \left(\vec{\delta} \cdot  \ai^{blp}  - \vec{\delta} \cdot  \ai^{blp} \right) \hat{e}_i \right] \cdot H \pdiv{\vec{c}}{z_k}
				\\
				& = 0
			\end{spalign}
			Therefore, when the BLUP obeys the constraints, it is a solution to the CLUP.
