...
\begin{document}
	...
	\section{Deriving the C-BLP}
		...
		\subsection{Inexact Constraints (Redux)}
			...
			\end{spalign}
			Since $\vec{q}$ is the BLP prediction we can once again see that the derivative is zero if the BLP obeys the constraints ($\vec{c} - D \vec{q} = 0$), so the C-BLP will always revert to the BLP if this meets our constraints. 
