...
\begin{document}
	...
	\section{The CLUP}
		...
		\begin{align}
			\begin{split}
			\pdiv{\mathcal{L}}{\ai} & = 2 K \ai - 2 \a \cdot \vec{k}_i + \Phi^T \tilde{\mu} +   \pdiv{\vec{p}}{\ai}(D^T\lambda)
			\\
		...
		\subsubsection{Applying Constraints}
			...
			\begin{spalign}
				...
				\\
				& = \frac{1}{2}{M^i}^{-1} \left( \Phi \vi + (D^T \vl) \cdot \hat{e}_i \Phi_t \wi - \vec{\varphi}_i\right)
			\end{spalign}
			And therefore
			\begin{spalign}
				\ai & = \left(\mathds{1} - K^{-1} \Phi^T{M^i}^{-1}\Phi \right)\vi +K^{-1} \Phi^T {M^i}^{-1} \vec{\varphi}_i + (D^T \vl) \cdot \hat{e}_i \left(\mathds{1} - K^{-1} \Phi^T {M^i}^{-1} \Phi \right)\wi 
				\\
				& = B^i \vi + C^i \vec{\varphi}_i + (D^T \vl) \cdot \hat{e}_i B^i \wi
				\\
				~
				\\
				& \text{where } C^i = K^{-1} \Phi^T{M^i}^{-1}
				\\
				& \text{and } B^i =  \left(\mathds{1} - C^i \Phi \right)
			\end{spalign}
			...
			\begin{spalign}
				...
				\\
				& = (B^i{\vi} + C^i \vec{\varphi}_i)\cdot \vec{X} + (D^T \vl) \cdot \hat{e}_i \vec{X} \cdot B^i K^{-1} \vec{X}
				\\
			...
			\begin{spalign}
				\ai = B^i \vi + C^i \vec{\varphi}_i + \left(D^T \left(D \mathcal{B} D^T \right)^{-1} \left(\vec{c} - D \vec{\alpha}\right)\right)\cdot \hat{e}_i B^i \wi
			\end{spalign}
			Where:
			\begin{spalign}
				\vi & = K^{-1} \vec{k}_i
			...
			We reformulate our Lagrangian in terms of $\vec{c}$ in order to allow us to optimise w.r.t. to inexact constraints. For convenience, we write the matrix product as $H = D^T(D \mathcal{B} D^T)^{-1}$

			\begin{spalign}
				\vec{k}_i \cdot \ai & = \vec{k}_i \cdot \left(B^i \vi + C^i \vec{\varphi}_i \right) + \left(H\vec{c} - H D \vec{\alpha} \right)\cdot \hat{e}_i \vec{k} \cdot B^i \vec{w}
				\\
				\ai \cdot K \ai &  = \left(B^i \vi + C^i \vec{\varphi}_i \right) \cdot K \left(B^i \vi + C^i \vec{\varphi}_i \right) 
				\\
				& ~~+ 2\left(H\vec{c} - H D \vec{\alpha} \right) \cdot \hat{e}_i ~ \times \wi^T B^i K\left(B^i \vi + C^i \vec{\varphi}_i \right)
				\\
			...
			\begin{spalign}
				...
				\\
				& + 2\left(\left(H\vec{c} - H D \vec{\alpha} \right) \cdot \hat{e}_i\right) \left(\wi^T B^i K\left(B^i \vi + C^i \vec{\varphi}_i \right) - \vec{k} \cdot B^i \vec{w}\right)
			\end{spalign}
			...
			Where:
			\begin{spalign}
				\ell_i = \frac{\wi^T B^i K\left(B^i \vi + C^i \vec{\varphi}_i \right) - \vec{k} \cdot B^i \vec{w}}{\vec{w}^T B^i K B^i \vec{w}}
			\end{spalign}
