...
\begin{document}
	...
	\section{The CLUP}
		...
		\begin{spalign}
			X_t - \hat{X}_t & = \left( \Phi \a - \vec{\varphi} \right) \cdot \vb + Y_t - \a \cdot \vec{Y}
		\end{spalign}
		Since $\E{Y} = \vec{0}$ and $\E{Y_t} = 0$ by definition, we note that the unbiased constraint is equal to:
		\begin{spalign}
			\E{X_t - \hat{X}_t} = 0 \LLR \left(\Phi\a - \vec{\varphi}\right) \cdot \E{\vec{\beta}} = 0
		\end{spalign}
		...
		\begin{spalign}
			\mu_t \mathcal{U}_t & = \left( \mu_t \E{\vec{\beta}} \right) \cdot \left(\Phi\a - \vec{\varphi}\right)
			\\
			& = \tilde{\mu}_t \cdot \left(\Phi\a - \vec{\varphi}\right)
		\end{spalign}
		...
		\begin{spalign}
			\mathcal{L}(\vec{a},\vec{\lambda},\{\tilde{\mu}\},\vec{c})& = \sum_{t \in T} \left(\E{X_t^2} + \a \cdot (K \a) - 2 \vec{k}_t \cdot \a + \tilde{\mu} \cdot \left(\Phi\a - \vec{\varphi}\right) \right)+ \vec{\lambda} \cdot \left( \vec{c} - D \vec{p} \right)
		\end{spalign}
		...
		\def\vl{\vec{\lambda}}
		\begin{spalign}
			\ai & = K^{-1} \ai - 2 K^{-1} \Phi_i^T \tilde{\mu}_i + K^{-1} Q_i D^T \vec{\lambda} ß
			\\
			& = \vi - 2 K^{-1}\Phi_i^T \tilde{\mu}_i + (D^T \vl) \cdot \hat{e}_i \wi \label{E:CLUP_a}
		\end{spalign}
		...
		\subsubsection{Applying Constraints}
			...
			\begin{spalign}
				\Phi_i \vi - 2 \Phi_i K^{-1} \Phi_i^T \tilde{\mu}_i + (D^T \vl) \cdot \hat{e}_i \Phi_t \wi = \vec{\varphi}
			\end{spalign}
			Solving for $\tilde{\mu}_i$:
			\begin{spalign}
				\tilde{\mu}_i & = \frac{1}{2} \left(\Phi_i K^{-1} \Phi_i^T\right)^{-1} \left( \Phi_i \vi + (D^T \vl) \cdot \hat{e}_i \Phi_t \wi - \varphi\right)
				\\
				& = \frac{1}{2}{M^i}^{-1} \left( \Phi_i \vi + (D^T \vl) \cdot \hat{e}_i \Phi_t \wi - \varphi\right)
			\end{spalign}
			And therefore
			\begin{spalign}
				\ai & = \left(\mathds{1} - K^{-1} \Phi_i^T{M^i}^{-1}\Phi_i \right)\vi +K^{-1} \Phi_i^T {M^i}^{-1} \varphi + (D^T \vl) \cdot \hat{e}_i \left(\mathds{1} - K^{-1} \Phi_i^T {M^i}^{-1} \Phi_i \right)\wi 
				\\
				& = B^i \vi + C^i \varphi + (D^T \vl) \cdot \hat{e}_i B^i \wi
				\\
				~
				\\
				& \text{where } C^i = K^{-1} \Phi_i^T{M^i}^{-1}
				\\
				& \text{and } B^i =  \left(\mathds{1} - C^i \Phi_i \right)
			\end{spalign}
			...
			\end{equation}
			Here $\otimes$ is the element-wise Hadamard product. Inserting this into the imposed constraints of \eref{E:CLUP_Constraint}, we find:
			\begin{spalign}
				D \vec{p} & = \vec{c}
				\\
				D \vec{\alpha} + D  \left(\vec{\beta} \otimes\Big( D^T \vl\Big) \right) = \vec{c}
				\\
				\vec{\beta} \otimes\Big( D^T \vl\Big) & = (D^T D)^{-1} \left(D^T\vec{c} - D^TD \vec{\alpha} \right)
				\\
				D^T \vl & = \left[ (D^T D)^{-1} D^T\vec{c} - \vec{\alpha} \right] \oslash \vec{\beta}
			\end{spalign}
			We can then insert this back into the definition of $\ai$ to find:
			\begin{spalign}
				\ai = B^i \vi + C^i \vec{\varphi} + \frac{\left((D^T D)^{-1} D^T\vec{c}\right)\cdot\hat{e}_i - \alpha_i }{\beta_i} B^i \wi
			\end{spalign}
			Where:
			\begin{spalign}
				\vi & = K^{-1} \vec{k}_i
				...
				\wi & = K^{-1} \vec{X}
				\\
				M^i & = \Phi_i K^{_1} \Phi_i^T
				\\
				C^i & = K^{-1} \Phi_i^T M^{i,-1}
				\\
				B^i & = \mathds{1} - C^i \Phi_i
				\\
