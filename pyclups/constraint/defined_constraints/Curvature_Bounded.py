...
class BoundedCurvature(Constraint):
	...
	def InitialiseConstraint(self,ts):
		...
		for i in range(0,len(s)-2):
			...
			if domainIdx[myIdx]:
				matrix[pIdx,myIdx] = 1
				matrix[pIdx,nextIdx] = -2
				matrix[pIdx,nextNext] = 1 
				pIdx +=1	
		...
		vector = OptimiseVector(constraintDim,constraintDim,lambda zs : dt*(lt-gt)/(1.0 + np.exp(-zs)), lambda zs: dt*(lt-gt)*np.exp(-zs)/(1 + np.exp(-zs))**2, lambda zs: -np.log(np.maximum(1e-8,np.divide(dt*(lt-gt),zs)-1)),gt*dt)
		vector.SetWBounds(-10,10)
		return vector,matrix
	
class MonotonicBoundedCurvature(Constraint):
	#it doesn't actually matter if valueBelow and valueAbove are switched around, the code makes sure that it works out in the end
	...
	def inverse(self,ps,lt,gt):
		...
		for i in range(1,len(ps)):
			...
			prev = lower + (upper-lower)/(1.0 + np.exp(-out[i]))
		t = self.transform(out,lt,gt)
		return out
	...
	def InitialiseConstraint(self,ts):
		...
		if callable(self.LessThan):
			ltt = ltt(ts)
		dt = float(ts[s[1]] - ts[s[0]])
		gt = np.minimum(gtt,ltt)*dt*dt
		lt = np.maximum(gtt,ltt)*dt*dt

		#assuming uniform absiscca here

		vector = OptimiseVector(constraintDim,constraintDim,lambda zs : self.transform(zs,lt,gt), lambda zs: self.derivative(zs,lt,gt), lambda zs: self.inverse(zs,lt,gt),0)
		vector.SetWBounds(-10,10)
		return vector,matrix
